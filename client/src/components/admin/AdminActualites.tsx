...
const EMPTY: Omit<ActualitePost, "id" | "date"> = {
  titre: "",
  slug: "",
  categorie: "",
  tags: [],
...
export default function AdminActualites() {
  const [posts, setPosts] = useState<ActualitePost[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<ActualitePost | null>(null);
  const [form, setForm] = useState<Omit<ActualitePost, "id" | "date">>(EMPTY);
  ...
  const fetchPosts = async () => {
    ...
    setError("");
    try {
      const res = await gasGet({ action: "getActualites" });
      if (res.ok) setPosts((res.data as ActualitePost[]) || []);
      else setError(res.error || "Erreur GAS");
    } catch {
      setError("Impossible de charger les actualités. Vérifiez la connexion GAS.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (post: ActualitePost) => {
    setEditing(post);
    setForm({ titre: post.titre, slug: post.slug, categorie: post.categorie, tags: post.tags, extrait: post.extrait, contenu: post.contenu, image: post.image, publie: post.publie });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const result = await uploadToCloudinary(imageFile, "actualites");
        imageUrl = result.url;
        setUploadingImg(false);
      }
      const payload = {
        action: "saveActualite",
  ...
  const handleDelete = async (post: ActualitePost) => {
    if (!confirm(`Supprimer "${post.titre}" ?`)) return;
    try {
      const res = await gasGet({ action: "deleteActualite", id: post.id });
    ...
    <div className="admin-section">
      <div className="admin-section__header">
        <div className="admin-section__info">
          <h2>Actualités</h2>
          <span className="admin-badge">{posts.length} actualité(s)</span>
        </div>
        <div className="admin-section__actions">
          <button className="admin-btn admin-btn--primary" onClick={openCreate}>+ Nouvelle actualité</button>
      ...
      {!loading && !error && posts.length === 0 && (
        <p className="admin-hint">⚠️ Aucune actualité — créez la première ou vérifiez VITE_GAS_URL.</p>
      )}
        ...
        <div className="admin-modal-overlay" onClick={e => { if (e.target === e.currentTarget) setShowForm(false); }}>
          <div className="admin-modal">
            <div className="admin-modal__header">
              <h3>{editing ? "Modifier l'actualité" : "Nouvelle actualité"}</h3>
      ...
      {!loading && !error && (
        <div className="admin-table-wrap">
          <table className="admin-table">
            ...
            <tbody>
              {posts.length === 0 ? (
                <tr><td colSpan={6} className="admin-empty">Aucune actualité — créez la première !</td></tr>
              ) : (
                posts.map(p => (
                  <tr key={p.id}>
                    <td>
                    ...
                    <td>
                      ...
                      <small className="admin-slug">/actualites/{p.slug}</small>
                    </td>
                    <td>{p.categorie || "—"}</td>
